    ...
    if (FindOrderDoc) 
    {
        ...
        if (text === expectedText) 
        {
            ...
            if (FindProduct) 
            {
                ...
                // เพิ่มเงื่อนไขการคำนวณภาษีแบบ แยกนอก <<<<<<<<<<<<<<<<<<<< 1 เพิ่มชาร์จ
                    //await page.pause();                    
                    await page.getByRole('tab', { name: 'ผู้จำหน่าย ' }).getByRole('button', { name: '' }).click();
                    await page.waitForTimeout(delay);
                    await page.getByRole('combobox', { name: 'ภาษีรวมใน' }).click();
                    await page.waitForTimeout(delay);
                    await page.locator('#bs-select-5-1').click(); //เลือกภาษีแยกนอก
                    await page.waitForTimeout(2000);
                    await page.locator('#ohdPrice1').fill('1000.00');
                    await page.waitForTimeout(delay);
                    await page.locator('#odvTBodyPOPdtAdvTableList > tr > td:nth-child(9) > div > button').click();
                    await page.waitForTimeout(delay);                    
                    //await page.pause();
                    //await page.getByRole('row', { name: '1 100.00 0.00 0.00 ชาร์จบาท Remove' }).getByRole('textbox').fill('50');
                    await page.locator('#otbPODisChgDataDocDTList > tbody > tr.xWPIDisChgTrTag > td:nth-child(7) > div > input').fill('50');
                ...
                // จบเพิ่มเงื่อนไขการคำนวณภาษีแบบ แยกนอก <<<<<<<<<<<<<<<<<<<< 1
                    await page.waitForTimeout(delay);
                    await page.getByRole('combobox', { name: 'ภาษีแยกนอก' }).click();
                    await page.waitForTimeout(delay);
                    await page.locator('#bs-select-5-0').click(); //เลือกภาษีรวมใน
                    await page.waitForTimeout(delay);
                // เพิ่มส่วนลดท้ายบิล คำนวณภาษีแบบรวมใน
                    console.log('\x1b[36m%s\x1b[0m',  'เพิ่มส่วนลดท้ายบิล 75');
                    await page.locator('#ohdPrice1').fill('1000.00');
                    await page.waitForTimeout(delay);
                    await page.locator('#odvTBodyPOPdtAdvTableList > tr > td:nth-child(9) > div > button').click();
                    await page.waitForTimeout(delay);    
                    ...
                    await page.getByRole('combobox', { name: 'ลดบาท' }).click();
                    await page.waitForTimeout(delay);
                    await page.locator('#bs-select-14-3 > span').click(); 
                    await page.waitForTimeout(delay);
                    await page.getByRole('combobox', { name: 'ชาร์ท %' }).click();
                    await page.waitForTimeout(delay);
                    await page.locator('#bs-select-14-0 > span').click();
                    await page.waitForTimeout(delay);
                    //await page.getByRole('row', { name: '1 50.00 0.00 0.00 ลดบาท Remove' }).getByRole('textbox').fill('75');
                    await page.locator('#otbPODisChgDataDocHDList > tbody > tr.xWPIDisChgTrTag > td:nth-child(7) > div > input').fill('75');
                    await page.waitForTimeout(3000);
                    await page.getByRole('button', { name: 'ตกลง' }).click();  // Error 03/07/2023 << กดตกลงแล้วตัวเลขจำนวนถูกรีเซ็ตเป็น 0
                    await page.waitForTimeout(3000);

                    //---------------------- ดึงข้อมูลราคามาแสดงก่อนทดสอบคำนวณภาษี
                    const valuepd = await page.inputValue('#ohdPrice1');
                    //const percentageString: string = valuepd;
                    console.log('ตัวเลขที่ได้ :', valuepd);                                                                        //Error 03/07/2023 ระบบเคลียร์ราคาต่อหน่วยเป็น 0
                    if (valuepd==='0.00'){
                        //await page.locator('#ohdPrice1').fill('1000.00');
                    }
                    else{
                    ...
                    await page.getByRole('combobox', { name: 'ลดบาท' }).click();
                    await page.waitForTimeout(delay);
                    await page.locator('#bs-select-16-3 > span').click(); 
                    await page.waitForTimeout(delay);
                    await page.getByRole('combobox', { name: 'ชาร์ท %' }).click();
                    await page.waitForTimeout(delay);
                    await page.locator('#bs-select-16-0 > span').click();
                    await page.waitForTimeout(delay);
                    //await page.getByRole('row', { name: '1 50.00 0.00 0.00 ลดบาท Remove' }).getByRole('textbox').fill('75');
                    await page.locator('#otbPODisChgDataDocHDList > tbody > tr.xWPIDisChgTrTag > td:nth-child(7) > div > input').fill('75');
                    await page.waitForTimeout(3000);
                    ...
                    if (ValueText) 
                    {
                        const text2 = await ValueText.innerText();
                        console.log('\x1b[36m%s\x1b[0m',  'VatFunction Testing : ', text2);
                        //console.log('\x1b[36m%s\x1b[0m',  'เพิ่มส่วนลดท้ายบิล คำนวณภาษีแบบรวมใน');
